...
type AttendanceChannel = "fingerprint" | "dni" | "phone";
...
export async function POST(req: Request) {
  try {
    ...
    const rawIdentifier =
      input.identifier ?? input.dni ?? input.document ?? input.phone;
    const identifierRaw =
      typeof rawIdentifier === "string" ? rawIdentifier : undefined;
    const intent: AttendanceIntent =
      input.intent === "checkout" ? "checkout" : "checkin";
    ...
    if (userId) {
      ...
      const res = await closeIfOpenOrCreate(user.id, intent, "fingerprint");
      if (!res.ok) {
    ...
    if (identifierRaw) {
      ...
      if (!isPhone && !isDni) {
        return NextResponse.json(
          {
            ok: false,
            message: "Ingresa un teléfono de 9 dígitos o DNI de 8 dígitos",
      ...
      const userIdFromIdentifier =
        await resolveUserIdByIdentifier(identifierRaw);
      ...
      const res = await closeIfOpenOrCreate(
